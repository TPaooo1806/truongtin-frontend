...
export default function AdminOrdersPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case 'PENDING_COD': 
      case 'PENDING_PAYOS': 
        return <span className="badge bg-warning text-dark px-3 py-2 rounded-pill shadow-sm"><i className="bi bi-hourglass-split me-1"></i> Chờ duyệt</span>;
      case 'PAID_PENDING_CONFIRM': 
        return <span className="badge bg-primary text-white px-3 py-2 rounded-pill shadow-sm"><i className="bi bi-credit-card me-1"></i> Đã thanh toán</span>;
      case 'PAID_AND_CONFIRMED': 
        return <span className="badge bg-success text-white px-3 py-2 rounded-pill shadow-sm"><i className="bi bi-check-circle-fill me-1"></i> Đã duyệt & Trừ kho</span>;
      case 'CANCELLED': 
        return <span className="badge bg-danger text-white px-3 py-2 rounded-pill shadow-sm"><i className="bi bi-x-circle-fill me-1"></i> Đã hủy</span>;
      default: 
        return <span className="badge bg-secondary px-3 py-2 rounded-pill shadow-sm">{status}</span>;
    }
  };

  if (loading) return <div className="text-center p-5 fw-bold">Đang tải đơn hàng...</div>;
    ...
    <div className="card border-0 shadow-sm rounded-4 p-4 bg-white">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <span className="text-muted small">Xem chi tiết và xác nhận trừ kho</span>
        </div>
      </div>
      ...
      <div className="table-responsive">
        <table className="table table-hover align-middle mb-0">
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.id}>
                <td className="ps-3 fw-bold text-primary">#{order.orderCode}</td>
                <td>
                  <div className="fw-bold text-dark">{order.customerName}</div>
                  <div className="text-muted small">{order.phone}</div>
                </td>
                <td className="text-secondary">{new Date(order.createdAt).toLocaleDateString('vi-VN')}</td>
        ...
        {orders.length === 0 && <div className="text-center p-4 text-muted">Không có đơn hàng nào.</div>}
      </div>

      {/* MODAL CHI TIẾT */}
      {selectedOrder && (
        <div className="modal d-block shadow-lg" style={{ backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 1050 }}>
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <div className="modal-content border-0 shadow rounded-4 overflow-hidden">
              <div className="modal-header bg-primary text-white border-0">
                <h5 className="modal-title fw-bold">Chi tiết đơn hàng #{selectedOrder.orderCode}</h5>
              ...
              <div className="modal-body p-4 bg-white">
                <div className="row mb-4 p-3 bg-light rounded-3 mx-0">
                  <div className="col-md-6 mb-3 mb-md-0">
                    <p className="mb-1 text-muted small text-uppercase fw-bold">Giao đến</p>
                    <p className="mb-0 fw-bold">{selectedOrder.customerName}</p>
                    <p className="mb-0 text-secondary small">{selectedOrder.phone}</p>
                    <p className="mb-0 text-secondary small">{selectedOrder.address}</p>
                  </div>
                ...
                <p className="mb-2 text-dark fw-bold"><i className="bi bi-box-seam me-2"></i>Sản phẩm đã chọn</p>
                <div className="table-responsive border rounded-3 shadow-sm mb-3">
                  <table className="table table-borderless mb-0">
                    ...
                    <tfoot className="table-light border-top">
                      <tr>
                        <td colSpan={2} className="ps-3 py-3 fw-bold text-dark">Tổng tiền</td>
